...
void PLX_CANBUS_configureViaPinSet(PLX_CANBUS_Handle_t aHandle, PLX_CANBUS_Unit_t aUnit, uint16_t aPinSet, const PLX_CANBUS_Params_t *aParams){
    ...
    obj->eCanShadow.CANMC.bit.SCB = 1; // configure in eCAN mode (all 32 mailboxes used)
    if(aParams->autoBusOn)
    {
        obj->eCanShadow.CANMC.bit.ABO = 1; // auto bus-on - tries to recover from bus-off state
    }
    else
    {
                         ...
                         uint32_t aId, bool isExtended, unsigned char aLen)
{
    ...
    if(isExtended)
    {
        ...
        obj->mailboxArray[aMailBox].MSGID.bit.STDMSGID = (uint16_t)((aId >> 18) & 0x7FF);
        obj->mailboxArray[aMailBox].MSGID.bit.EXTMSGID_H = (uint16_t)((aId >> 16) & 0x3);
...
bool PLX_CANBUS_isBusOn(PLX_CANBUS_Handle_t aHandle){
    ...
    obj->eCanShadow.CANES.all = obj->eCanRegsPtr->CANES.all;
    obj->eCanShadow.CANMC.all = obj->eCanRegsPtr->CANMC.all;
    return ((obj->eCanShadow.CANES.bit.BO == 0) && (obj->eCanShadow.CANMC.bit.CCR == 0));
}
...
bool PLX_CANBUS_getMessage(PLX_CANBUS_Handle_t aHandle, uint16_t aMailBox, unsigned char data[], unsigned char lenMax){
    ...
    obj->eCanShadow.CANRMP.all = obj->eCanRegsPtr->CANRMP.all;
    // we have to loop to make sure message is not changed
    // while we read it
    while((obj->eCanShadow.CANRMP.all & mask) != 0 ){
        ...
        // read data
        unsigned char d[] = {
            obj->mailboxArray[aMailBox].MDL.byte.BYTE0,
...
bool PLX_CANBUS_putMessage(PLX_CANBUS_Handle_t aHandle, uint16_t aMailBox, const unsigned char data[], unsigned char len){
    ...
    obj->eCanShadow.CANTRS.all = obj->eCanRegsPtr->CANTRS.all;
    if((obj->eCanShadow.CANTRS.all & aMailBox) != 0){
        // tx mailbox is busy!
        return false;
    }
