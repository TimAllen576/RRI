...
void PLX_QEP_configureViaPinSet(PLX_QEP_Handle_t aHandle, uint16_t aUnit, uint16_t aPinSet, const PLX_QEP_Params_t *aParams)
{
	...
	switch(aUnit)
	{
    // omitted code for "internal pull-up enable" (e.g., GpioCtrlRegs.GPAPUD.bit.GPIO20 = 0)
    // & "synchronize inputs to SYSCLKOUT" (e.g., GpioCtrlRegs.GPAQSEL2.bit.GPIO20 = 0)
    // if the selected pins are already set to the required value by default.
		default:
		case 1:
			obj->qep = &EQep1Regs;
