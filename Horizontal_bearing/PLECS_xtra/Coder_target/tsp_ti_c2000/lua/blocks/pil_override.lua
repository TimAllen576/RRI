...
function Module.getBlock(globals)
  ...
  local PilOverride = require('blocks.block').getBlock(globals)
  PilOverride["instance"] = static.numInstances
  static.numInstances = static.numInstances + 1

  function PilOverride:checkMaskParameters(env)
  end

  function PilOverride:getDirectFeedthroughCode()
    ...
    for i = 1, #Block.InputSignal[1] do
      ...
      pil_obj:registerOverrideProbe(name, {type = Block.InputType[1][i]})

      OutputCode:append("SET_OPROBE(%s_probes.%s, %s);" %
                            {
    ...
    return {
      ...
      Require = Require,
      UserData = {bid = PilOverride:getId()}
    }
  end

  function PilOverride:finalize(c)
    return c
  end
